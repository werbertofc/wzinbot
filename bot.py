...
DESTINOS_BR_FILE = 'destinos_br.json'  # NOVO!
...
acesso = {}
grupos = {}
destinos = set()           # Conjunto de destinos normais
destinos_br = set()        # Conjunto de destinos BR
bots_oficiais = set()      # Conjunto de bots normais
bots_br = set()            # Conjunto de bots BR
delay = 30
delay_br = 30
last_like = {}
last_like_br = {}
    ...
    for cid in destinos:
        for bot_id in bots_oficiais:
            try:
                ...
                print(f"[LOG] /like → {cid} (bot {bot_id})")
                enviados += 1
    ...
    for cid in destinos_br:
        for bot_id in bots_br:
            try:
                ...
                print(f"[LOG] /like BR → {cid} (bot {bot_id})")
                enviados += 1
                await asyncio.sleep(0.3)
            except: pass
...
async def resposta(e):
    ...
    tipo, bot_esperado = pending_responses[key][2], pending_responses[key][3]
...
# INICIAR
# ========================================
print(f"BOT INICIADO! | Bots: {len(bots_oficiais)} | Bots BR: {len(bots_br)} | Destinos: {len(destinos)} | BR: {len(destinos_br)}")
client.start()
